<div class="bg-dark flex h-[65px] w-full items-center justify-between pl-5">
  <a class="bg-logo h-[25px] w-[50px] bg-cover" href="{{route('home')}}"></a>
  <section class="flex gap-12 h-full font:pop z-50">
    <ul class="flex gap-4 hover:text-white h-full">
      <li class="group relative h-full flex items-center w-16 justify-center">
          ...
          @else
            ...
            <li class="uppercase w-72">
              <a href="{{ route('logout') }}" onclick="event.preventDefault();
                ...
                class=" block w-full text-right px-6 py-2 font-semibold text-principal font-pop duration-300 hover:text-dark hover:bg-principal hover:cursor-pointer">
                {{ __('Logout') }}
              </a>
